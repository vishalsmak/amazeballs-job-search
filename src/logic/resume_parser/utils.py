...
def extract_experience(resume_text):
    ...
    filtered_sentence = [
        ...
        and wordnet_lemmatizer.lemmatize(w) not in stop_words
    ]
    ...
    for vp in list(cs.subtrees(filter=lambda x: x.label() == "P")):
        test.append(
            " ".join([i[0] for i in vp.leaves() if len(vp.leaves()) >= 2])
        )
...
def extract_measurable_results(text, experience_list):
    ...
    experience_text = " ".join(
        [text[: len(text) // 2 - 1] for text in experience_list]
    )
    mr_dict = {}
