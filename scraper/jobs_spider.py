...
class JobsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        page_count += 1
        print(
            f"Onto Page number : {page_count} , jobs scraped : {job_count}, remaining jobs to scrape : {total_jobs - job_count}"
        )
    ...
    def GetJobCount(self, response):
        try:
            return int(
                response.xpath('.//div[@id="searchCountPages"]')
                .extract_first()
                .split(" ")[-2]
                .replace(",", "")
            )
    ...
    def GetCompany(self, job):
        try:
            company = job.xpath(
                './/div[@class="jobsearch-CompanyInfoContainer"]//a/text()'
            ).extract_first()
            if company:
                return company
            else:
                return job.xpath(
                    './/div[@class="jobsearch-CompanyInfoContainer"]//div[text()]/text()'
                ).extract_first()
        except:
            return "NOT_FOUND"
